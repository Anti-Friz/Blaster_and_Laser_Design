...
namespace Blaster_and_Laser_Design
{
    class Desinger
    {
        double GetEmptyWeight(double d, TLs tl, Beams beam, Focus focus, Generators generator, Barells barell = 0)
        {
            double s, e = 0, f = 0, g = 0, b = 1;
            ...
            {
                return Math.Pow(d * s * 3.7, 3) * f;
            }

            return Math.Pow(((d * s) / e), 3) * f * g * b;
        }
        public string RoundDamage(double d)
        {
            if (d < 1)
            {
                if (d > 0 && d <= 0.032)
        ...
        public int GetCost(double emptyWeight, Beams beam, Generators generator)
        {
            int bc = 0, gc  = 0;

            return (int)Math.Round(emptyWeight * bc * gc);
        }
